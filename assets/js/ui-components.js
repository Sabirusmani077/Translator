import { getSiteData } from "./storage.js?v=20260405-local-home-2";
...
function renderHeaderProfileShortcut() {
  return "";
}
            ...
            .map(
              (item) => `
                <a class="main-nav__link ${pageId === item.id ? "is-active" : ""} ${
                  item.protected ? "is-locked" : ""
                }" href="${item.href}">
                  ${item.label}
                  ${item.protected ? '<span class="nav-lock">Access</span>' : ""}
                </a>
              `
            )
            .join("")}
...
export function renderSiteFooter() {
  ...
  const quickLinks = [
    { label: "Translator", href: "translator.html" },
  ...
  return `
    <div class="footer-grid footer-grid--compact">
      ...
      <div class="footer-panel footer-panel--compact">
        <h3>Quick Access</h3>
      ...
      <div class="footer-panel footer-panel--compact">
        ...
        <p class="footer-note">Classes, feedback, aur learning support ke liye directly contact kar sakte hain.</p>
      </div>
    </div>
