...
function normalizeInput(value) {
  return String(value || "")
    ...
    .replace(/\bmerae\b/g, "mere")
    .replace(/\bmereko\b/g, "mujhe")
    .replace(/\bmujhko\b/g, "mujhe")
    .replace(/\bbolne mein\b/g, "bolne me")
    .replace(/\bmien\b/g, "me")
    .replace(/\btha na\b/g, "tha")
    .replace(/\bhai na\b/g, "hai")
    .replace(/\bja rha\b/g, "ja raha")
    .replace(/\bjaa rha\b/g, "ja raha")
    .replace(/\bkar rha\b/g, "kar raha")
    .replace(/\bbol rha\b/g, "bol raha")
...
function prepareTranslationInput(inputValue, mode = "hi-to-en") {
  let prepared = String(inputValue || "")
    .replace(/[â€œâ€]/g, '"')
    .replace(/[â€˜â€™]/g, "'")
    .replace(/\u0964/g, ".")
    .replace(/\s+/g, " ")
  ...
  if (mode === "hi-to-en" && !/[\u0900-\u097F]/.test(prepared)) {
    const romanHindiFixes = [
      [/\bmai\b/gi, "main"],
      [/\bme\b/gi, "main"],
      [/\bmereko\b/gi, "mujhe"],
      [/\bmujhko\b/gi, "mujhe"],
      [/\bkar rha\b/gi, "kar raha"],
      [/\bja rha\b/gi, "ja raha"],
      [/\bbol rha\b/gi, "bol raha"],
      [/\bbahot\b/gi, "bahut"],
      [/\bkosis\b/gi, "koshish"],
      [/\bkyu\b/gi, "kyun"],
      [/\bwha\b/gi, "waha"],
    ...
    romanHindiFixes.forEach(([pattern, replacement]) => {
...
function cleanTranslatedSentence(text, mode) {
  const cleaned = String(text || "")
    .replace(/\s+/g, " ")
    .replace(/\s+([,.!?])/g, "$1")
    .trim();

  return mode === "en-to-hi" ? cleanHindiSentence(cleaned) : cleanSentence(cleaned);
}
...
async function fetchGoogleStyleTranslation(inputValue, mode) {
  ...
  endpoint.searchParams.set("dt", "t");
  endpoint.searchParams.set("dj", "1");
  endpoint.searchParams.set("q", prepareTranslationInput(inputValue, mode));
...
function buildLiveResult(inputValue, liveTranslation, mode, matchedPhrase = null) {
  if (mode === "en-to-hi") {
    const directHindi = cleanTranslatedSentence(liveTranslation.translatedText, mode);
  ...
  const directEnglish = cleanTranslatedSentence(liveTranslation.translatedText, mode);
...
export async function translateText(inputValue, siteData, mode = "hi-to-en") {
  ...
  const preparedInput = prepareTranslationInput(inputValue, activeMode);
  const matchedPhrase =
    activeMode === "en-to-hi"
      ? findEnglishPhraseMatch(siteData, preparedInput)
      : findHindiPhraseMatch(siteData, preparedInput);

  try {
    const liveTranslation = await fetchGoogleStyleTranslation(preparedInput, activeMode);
    return buildLiveResult(preparedInput, liveTranslation, activeMode, matchedPhrase);
  } catch (error) {
    return buildFallbackResult(preparedInput, activeMode, matchedPhrase);
  }
}
