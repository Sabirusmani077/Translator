import { refreshReveal, showToast } from "./app.js?v=20260405-local-practice-1";
import {
  getDailyChallenge,
  getFavorites,
  getCurrentUser,
  ...
  togglePracticeItem,
} from "./storage.js?v=20260405-local-practice-1";

document.addEventListener("DOMContentLoaded", () => {
  const currentUser = getCurrentUser();
  const practiceOwnerId = currentUser?.id || "guest-practice";
  const root = document.getElementById("page-root");
  const siteData = getSiteData();
  const challenge = getDailyChallenge();
  const phraseOfDay = getPhraseOfTheDay();
  let savedResponses = getSpeakingPracticeResponses(practiceOwnerId);
  ...
  function renderPage() {
    ...
    root.querySelectorAll("[data-save-answer]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        reviewState[questionId] = evaluateAnswer(answer, question);
        saveSpeakingPracticeResponse(practiceOwnerId, questionId, answer);
        savedResponses = getSpeakingPracticeResponses(practiceOwnerId);
